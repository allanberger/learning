...
The `home-routes` handler in the `fb-bot.routes.home` namespace
...
The `home-page` function will in turn call the `fb-bot.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `fb-bot.handler` namespace:
...
Request middleware functions are located under the `fb-bot.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `fb-bot.dev-middleware` namespace found in
